...
void OSCInterface::itemsAdded(Array<OSCOutput*> outputs)
{
	...
	try
	{
		...
		for (int i = 1; i < a.numArguments; ++i)
		{
			if (a.arguments[i].isArray())
			{
				...
				for (auto& aa : *arr) msg.addArgument(OSCHelpers::varToArgument(aa, OSCHelpers::Float));
			}
			else
			{
				msg.addArgument(OSCHelpers::varToArgument(a.arguments[i], OSCHelpers::Float));
			}
		}
