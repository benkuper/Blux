...
namespace
{
    ...
    class ScriptColorSourceScript :
        public Script
    {
    public:
        ScriptColorSourceScript(ScriptTarget* parentTarget, bool canBeDisabled, bool canBeRemoved) :
            Script(parentTarget, canBeDisabled, canBeRemoved)
        {
            scriptObject.getDynamicObject()->setMethod("noise", &ScriptColorSourceScript::noiseFromScript);
        }
        ...
        void onContainerParameterChangedInternal(Parameter* p) override
        {
            if (p == filePath)
            {
                ObjectManager::ScopedProcessSuspender processSuspender("Script color source reload");
...
{
    script.reset(new ScriptColorSourceScript(this, false, false));
    script->setCustomShortName("script");
    script->setParamsContainer(&sourceParams, false, false);
...
{
    if (script == nullptr || script->state != Script::SCRIPT_LOADED)
    {
    ...
    colorData.colorArray.resize(colors.size());
    colorData.colorArray.fill(Colours::black);
    ...
    args.add(comp != nullptr ? comp->getScriptObject() : var());
